...
Uint8 calcJulia(unsigned x, unsigned y);
...
/*The difference between the julia set and the mandelbrot set is that the z and c parts of of sets are switched*/
Uint8 calcJulia(unsigned x, unsigned y)
{
	struct complex z;
	struct complex c;
	...
	z.r = (((((float)x / (float)WIDTH) * 3.5) - 2.0) * 1.3);
	z.i = (((((float)y / (float)HEIGHT) * 2.0) - 1.0) * 1.3);
	c.r = -.1;
	c.i = .651;
	
	/*printf("x:%d y:%d fx:%f fy:%f r:%f i:%f\n", x, y, fx, fy, c.r, c.i);*/
